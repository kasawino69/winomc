...
WinoMC nutzt eine eigene native Runtime und ist nicht mehr vom `itzg`-Image abhängig.
...
WinoMC enthält eine eigene Web-Konsole direkt in Home Assistant.
...
WinoMC nutzt zwei klare Ansichten:
...
WinoMC enthält Optionen, um den Server besser an das Hostsystem anzupassen.

| Einstellung | Bedeutung |
|---|---|
| `WINOMC_HOST_OPTIMIZATION_MODE` | Optimierungsprofil für das Hostsystem. |
| `WINOMC_CPU_AFFINITY` | Optional feste CPU-Zuordnung. Für normale Nutzer leer lassen. |
| `WINOMC_BDS_NICE` | Prozess-Priorität. Normalerweise `auto`. |
| `WINOMC_BDS_IONICE_CLASS` | I/O-Priorität. Normalerweise `auto`. |
| `WINOMC_BDS_IONICE_LEVEL` | I/O-Level. Normalerweise `auto`. |
| `WINOMC_ENABLE_JEMALLOC` | Speicheroptimierung. Normalerweise `auto`. |
| `WINOMC_RUNTIME_ANALYSIS` | Zeigt Laufzeit- und Systemhinweise an. |

Für die meisten Nutzer ist diese Einstellung sinnvoll:

```yaml
WINOMC_HOST_OPTIMIZATION_MODE: balanced
```
...
Der IPv6-Bind-Fix wird von WinoMC selbst aus Quellcode gebaut.  
...
## Optionale Ergänzung: WinoMC Friend Broadcast
